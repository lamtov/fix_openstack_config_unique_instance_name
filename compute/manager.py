...
###################################### lamtv10 start edit here ############################
    ...
    def _post_live_migration(self, ctxt, instance,
        ...
        #self.log_db_instance_extra(instance,"lamtv10 BEFORE CLEEANUP ")

        LOG.debug("lamtv10_________   do_cleanup %s", do_cleanup)
        if do_cleanup:
        ...
        #self.log_db_instance_extra(instance,"lamtv10 AFTER CLEEANUP ")
        # NOTE(timello): make sure we update available resources on source
        # host even before next periodic task.
        #self._log_db_compute_source("lamtv10 BEFORE UPDATE AVAILABLE RESOURCE ")
        self.update_available_resource(ctxt)

        try:
            LOG.debug("lamtv10_________     update %s ", ctxt)
            #self._log_db_compute_source("lamtv10 AFTER UPDATE AVAILABLE RESOURCE ")

            LOG.info("START update instance database")
            json_instance_numa_topology=  jsonutils.loads(instance.numa_topology._to_json())
            if json_instance_numa_topology['nova_object.data']['cells'][0]["nova_object.data"]['cpu_policy']=='dedicated' :
                self.update_db_instance_extra(instance,dest)
            #self.log_db_instance_extra(instance,"lamtv10 BEFORE FINAL  ")
        except Exception as error:
            LOG.exception("Update db failed for instance %s", instance , error=error)

        #self._log_db_compute_source("lamtv10 BEFORE UPDATE _update_scheduler_instance_info ")
        self._update_scheduler_instance_info(ctxt, instance)
        #self._log_db_compute_source("lamtv10 AFTER UPDATE _update_scheduler_instance_info ")   


        #self._log_db_compute_source("lamtv10 BEFORE UPDATE _notify_instance_info ")     
        self._notify_about_instance_usage(ctxt, instance,
                                          "live_migration._post.end",
                                          network_info=network_info)
        #self._log_db_compute_source("lamtv10 AFTER UPDATE _notify_instance_info ")
        ...
        #self.log_db_instance_extra(instance,"lamtv10 AFTER FINAL  ")
    ...
    #def _log_db_compute_source(self,xxxxxx) :
        #engine = db.create_engine(CONF.database.connection)
        #connection = engine.connect()
        #metadata = db.MetaData()
        #nova_compute_nodes = db.Table('compute_nodes', metadata, autoload=True, autoload_with=engine)

        #compute_cell= jsonutils.loads(connection.execute(db.select([nova_compute_nodes.columns.numa_topology]).where(nova_compute_nodes.columns.hypervisor_hostname==self.host)).fetchall()[0][0])
        #LOG.info("lamtv10___ DB_COMPUTE_ xxxxxx: %s %s ", xxxxxx,   compute_cell["nova_object.data"]["cells"][0]["nova_object.data"]["pinned_cpus"])
    ...
    def post_live_migration_at_destination(self, context, instance,
        ...
        try:
            ...
            LOG.info("lamtv10_________ cpu_pinning_raw: %s  host   %s" , instance.numa_topology.cells[0].cpu_pinning_raw, self.host)

            json_instance_numa_topology=  jsonutils.loads(instance.numa_topology._to_json())
            if json_instance_numa_topology['nova_object.data']['cells'][0]["nova_object.data"]['cpu_policy']=='dedicated' :
                self._reset_change_instance_numa_topology(instance, self.host)
            ...
            try:
                ...
                self.log_db_instance_extra(instance, 'BEFORE FINAL OF DESSTINATION')
                instance.host = self.host
                instance.power_state = current_power_state
                instance.task_state = None
...
#        self.log_db_instance_extra(instance, 'BEFORE NOTIFY ')
        self._notify_about_instance_usage(
                     context, instance, "live_migration.post.dest.end",
                     network_info=network_info)
#        self.log_db_instance_extra(instance, 'AFTER NOTIFY')

    def _reset_change_instance_numa_topology(self,instance, host) :

        for i in range(0,len(instance.numa_topology.cells)):
            cell = instance.numa_topology.cells[i]
            if cell.get('cpu_pinning_raw') :
                if len(cell['cpu_pinning_raw']) >0 :
                    size_vcpus_instance = len(cell.cpu_pinning_raw)
                    size_ram_instance=instance.memory_mb
                    db_engine = db.create_engine(CONF.database.connection)
                    db_connection = db_engine.connect()
                    db_metadata = db.MetaData()
                    db_nova_compute_nodes = db.Table('compute_nodes', db_metadata, autoload=True, autoload_with=db_engine)
                    compute_numa_topology = jsonutils.loads(db_connection.execute(db.select([db_nova_compute_nodes.columns.numa_topology]).where(db_nova_compute_nodes.columns.hypervisor_hostname==host)).fetchall()[0][0])
                    is_need_change = True

                    for numa_cell in compute_numa_topology["nova_object.data"]["cells"]:
                        len_cpuset= len(numa_cell["nova_object.data"]["cpuset"])
                        numa_cell_pinned_cpus=numa_cell["nova_object.data"]["pinned_cpus"];
                        len_pinned_cpus= len(numa_cell_pinned_cpus)
                        size_ram_numa_cell= numa_cell["nova_object.data"]["memory"]
                        used_ram_numa_cell= numa_cell["nova_object.data"]["memory_usage"]
                        if len_cpuset - len_pinned_cpus >= size_vcpus_instance and size_ram_numa_cell - used_ram_numa_cell >= size_ram_instance and is_need_change == True :

                            numa_cell_siblings = numa_cell["nova_object.data"]["siblings"]
                            new_pinned_cpu = []
                            numa_node_id=numa_cell["nova_object.data"]["id"]
                            for sibling in numa_cell_siblings :
                                if sibling[0] not in numa_cell_pinned_cpus and sibling[1] not in numa_cell_pinned_cpus  and len(new_pinned_cpu) != size_vcpus_instance :
                                    new_pinned_cpu.append(sibling[0])
                                    new_pinned_cpu.append(sibling[1])
                            for j in range(0,size_vcpus_instance):
                                instance.numa_topology.cells[i].cpu_pinning_raw[str(j)]=new_pinned_cpu[j]
                                instance.numa_topology.cells[i].id=numa_node_id
                                #LOG.error("lamtv10_________ NEW INSTANCE  i  : %s ---  %s" ,j, instance.numa_topology.cells[0].cpu_pinning_raw)
                            is_need_change = False

        self.log_db_instance_extra(instance, 'BEFORE SAVE')
        LOG.debug("lamtv10_________ NEW INSTANCE  BEFORE SAVE : %s" , instance.numa_topology.cells[0].cpu_pinning_raw)

        instance.save()

        self.log_db_instance_extra(instance, 'AFTER SAVE')
        LOG.debug("lamtv10_________ NEW INSTANCE: %s" , instance.numa_topology.cells[0].cpu_pinning_raw)


    def log_db_instance_extra(self,instance,xxxxxx):
        db_engine = db.create_engine(CONF.database.connection)
        db_connection = db_engine.connect()
        db_metadata = db.MetaData()
        nova_instance_extra=db.Table('instance_extra',db_metadata, autoload=True, autoload_with=db_engine)
        instances_uuid= str(instance.uuid)
        instance_info=jsonutils.loads(db_connection.execute(db.select([nova_instance_extra.columns.numa_topology]).where(nova_instance_extra.columns.instance_uuid==instances_uuid)).fetchall()[0][0])
        LOG.debug("lamtv10_________ INSTANCE _INFO %s : %s", xxxxxx ,  instance_info["nova_object.data"]["cells"][0]['nova_object.data']['cpu_pinning_raw']    )

    def is_migrate_ceph(self):
        db_engine = db.create_engine(CONF.database.connection)
        db_connection = db_engine.connect()
        db_metadata = db.MetaData()
        quannt24=db.Table('quannt24',db_metadata, autoload=True, autoload_with=db_engine)
        is_migrate_ceph = db_connection.execute(db.select([quannt24.columns.is_migrate_ceph]).where(quannt24.columns.id==1)).fetchall()[0][0]
        LOG.error("lamtv10_________ is_migrate_ceph: %s" is_migrate_ceph)
        return is_migrate_ceph 
        # is_migrate_ceph = self.is_migrate_ceph()

    def update_db_instance_extra(self,instance, host):
        for i in range(0,len(instance.numa_topology.cells)):
            cell = instance.numa_topology.cells[i]
            if cell.get('cpu_pinning_raw') :
                if len(cell['cpu_pinning_raw']) >0 :
                    ...
                    compute_numa_topology = jsonutils.loads(db_connection.execute(db.select([db_nova_compute_nodes.columns.numa_topology]).where(db_nova_compute_nodes.columns.hypervisor_hostname==host)).fetchall()[0][0])
                    is_need_change = True
                    #DROP TABLE IF EXISTS `instance_extra_lamtv10`
                    nova_instance_extra=db.Table('instance_extra',db_metadata, autoload=True, autoload_with=db_engine)
                    instances_uuid= str(instance.uuid)
                    instance_info=jsonutils.loads(db_connection.execute(db.select([nova_instance_extra.columns.numa_topology]).where(nova_instance_extra.columns.instance_uuid==instances_uuid)).fetchall()[0][0])
                        ...
                        if len_cpuset - len_pinned_cpus >= size_vcpus_instance and size_ram_numa_cell - used_ram_numa_cell >= size_ram_instance and is_need_change == True :
                            ...
                            for sibling in numa_cell_siblings :
                                if sibling[0] not in numa_cell_pinned_cpus and sibling[1] not in numa_cell_pinned_cpus  and len(new_pinned_cpu) != size_vcpus_instance :
                                    new_pinned_cpu.append(sibling[0])
                                    new_pinned_cpu.append(sibling[1])
                            for j in range(0, len(new_pinned_cpu)):
                                instance_info["nova_object.data"]["cells"][0]['nova_object.data']['cpu_pinning_raw'][str(j)]=new_pinned_cpu[j]
                            instance_info["nova_object.data"]["cells"][0]['nova_object.data']['id']=numa_node_id

                            update_cpu_pinned = nova_instance_extra.update().where(nova_instance_extra.columns.instance_uuid==instances_uuid).values(numa_topology=jsonutils.dumps(instance_info))
                            db_connection.execute(update_cpu_pinned)
                            LOG.info("lamtv10 update db instance to: %s",   instance_info["nova_object.data"]["cells"][0]['nova_object.data']['cpu_pinning_raw']    )
                            is_need_change = False
